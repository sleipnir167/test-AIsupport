...
function LogCard({ entry }: { entry: AILogEntry }) {
  ...
  const typeLabel = entry.type === 'generation'
    ? (entry.logStage === 'planning' ? '📋 プランニング' : entry.logStage === 'batch' ? '⚡ バッチ実行' : entry.logStage === 'system_analysis' ? '🔍 システム分析' : 'テスト生成')
...
export default function AILogsPage({ params }: { params: { id: string } }) {
  ...
  const [filterType, setFilterType] = useState<'all' | 'planning' | 'batch' | 'system_analysis' | 'generation' | 'review' | 'compare'>('all')
